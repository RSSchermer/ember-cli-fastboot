...
module.exports = function fastbootAppModule(prefix, configAppAsString) {
  return [
    ...
    "  if (!runningTests) {",
    "    require('{{MODULE_PREFIX}}/src/main')['default'].create({{CONFIG_APP}});",
    "  }",
    "}",
    "",
    "define('~fastboot/app-factory', ['{{MODULE_PREFIX}}/src/main', '{{MODULE_PREFIX}}/config/environment'], function(App, config) {",
    "  App = App['default'];",
